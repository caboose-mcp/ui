...
export const TOOLS: ToolDef[] = [
  ...
  // ── Auth & RBAC ────────────────────────────────────────────────────────────
  {
    ...
    tier: 'hosted',
    tags: ['security', 'write'],
    description: 'Create a JWT token for API access or a magic link for web login.',
    params: [
      { name: 'label', type: 'string', required: true, description: 'Friendly name for the token (e.g. "claude-code on pi")' },
      { name: 'tools', type: 'string', required: false, description: 'Comma-separated tool names (empty = all tools)' },
      { name: 'expires_days', type: 'number', required: false, description: 'Days until token expires (default: 30)', default: '30' },
    ],
    example: { input: { label: 'my-token', expires_days: 90 }, output: 'Token created: eye...xyz (expires 2026-06-19)' },
  },
  {
    name: 'auth_list_tokens',
    category: 'auth',
    ...
    description: 'List all issued JWT tokens with their labels, created date, expiry, and last used timestamp.',
    params: [],
    example: { input: {}, output: '3 tokens:\n• my-token (created 2026-03-10, expires 2026-06-19, used 2m ago)\n• pi-backup (created 2026-02-01, expires 2026-05-02, never used)' },
  },
  {
    name: 'auth_revoke_token',
    category: 'auth',
    tier: 'hosted',
    tags: ['security', 'write'],
    description: 'Immediately revoke a token, making it invalid for future requests.',
    params: [
      { name: 'id', type: 'string', required: true, description: 'Token ID from auth_list_tokens' },
    ],
    example: { input: { id: 'tok-abc123' }, output: 'Token tok-abc123 revoked and blacklisted.' },
  },
  {
    name: 'auth_link_identity',
    category: 'auth',
    tier: 'hosted',
    tags: ['security', 'write'],
    description: 'Link a platform identity (Discord/Slack user ID) to a token for automatic authentication.',
    params: [
      { name: 'token_id', type: 'string', required: true, description: 'Token ID to link' },
      { name: 'platform', type: 'string', required: true, description: 'Platform: discord or slack' },
      { name: 'platform_id', type: 'string', required: true, description: 'User ID on the platform' },
    ],
    example: { input: { token_id: 'tok-abc123', platform: 'discord', platform_id: 'U12345' }, output: 'Linked Discord user U12345 to token tok-abc123.' },
  },
  {
    name: 'auth_list_identities',
    category: 'auth',
    tier: 'hosted',
    tags: ['security', 'read-only'],
    description: 'List all linked platform identities (Discord/Slack users) and their associated tokens.',
    params: [],
    example: { input: {}, output: '2 linked identities:\n• Discord: U12345 → tok-abc123 (my-token)\n• Slack: U67890 → tok-xyz789 (slack-bot)' },
  },
  {
    name: 'auth_unlink_identity',
    category: 'auth',
    tier: 'hosted',
    tags: ['security', 'write'],
    description: 'Unlink a platform identity from a token.',
    params: [
      { name: 'token_id', type: 'string', required: true, description: 'Token ID' },
      { name: 'platform', type: 'string', required: true, description: 'Platform: discord or slack' },
    ],
    example: { input: { token_id: 'tok-abc123', platform: 'discord' }, output: 'Unlinked Discord identity from token tok-abc123.' },
  },
  ...
  // ── Notes ──────────────────────────────────────────────────────────────────
  {
    ...
    tier: 'hosted',
    tags: ['write'],
    description: 'Append a quick timestamped note to ~/.claude/notes.md with an optional tag/category.',
    params: [
      { name: 'text', type: 'string', required: true, description: 'Note content' },
      { name: 'tag', type: 'string', required: false, description: 'Optional tag/category (e.g. idea, todo, bug)' },
    ],
    example: { input: { text: 'Investigate JWT refresh token rotation', tag: 'todo' }, output: 'Note saved (240 chars).' },
  },
  {
    name: 'note_list',
    category: 'notes',
    tier: 'hosted',
    tags: ['read-only'],
    description: 'List recent notes from ~/.claude/notes.md, optionally filtered by tag.',
    params: [
      { name: 'limit', type: 'number', required: false, description: 'Max notes to return (default 10)', default: '10' },
      { name: 'tag', type: 'string', required: false, description: 'Filter by tag (e.g. todo, bug)' },
    ],
    example: { input: { limit: 5, tag: 'todo' }, output: '[2026-03-19 14:32] Investigate JWT refresh token rotation\n[2026-03-18 10:15] Review auth flow docs' },
  },
  {
    name: 'notes_drive_backup',
    category: 'notes',
    tier: 'hosted',
    tags: ['backup', 'google', 'write'],
    description: 'Upload ~/.claude/notes.md to Google Drive (requires Google auth).',
    params: [
      { name: 'folder_id', type: 'string', required: false, description: 'Google Drive folder ID (default: root)' },
    ],
    example: { input: {}, output: 'Backed up notes.md to Google Drive (2.5 KB).' },
  },
  {
    name: 'notes_drive_restore',
    category: 'notes',
    tier: 'hosted',
    tags: ['backup', 'google', 'write'],
    description: 'Download notes.md from Google Drive and restore it locally.',
    params: [
      { name: 'folder_id', type: 'string', required: false, description: 'Google Drive folder ID (default: root)' },
    ],
    example: { input: {}, output: 'Restored notes.md from Google Drive (2.5 KB, 847 lines).' },
  },
  ...
  // ── Focus ──────────────────────────────────────────────────────────────────
  {
    ...
    category: 'focus',
    tier: 'hosted',
    tags: ['productivity', 'write'],
    description: 'Start a focus session with a declared goal and optional timer (default 60 minutes).',
    params: [
      { name: 'goal', type: 'string', required: true, description: 'Session goal (e.g. "Debug auth flow")' },
      { name: 'duration_minutes', type: 'number', required: false, description: 'Session duration in minutes (default 60)', default: '60' },
    ],
    example: { input: { goal: 'Implement JWT refresh', duration_minutes: 90 }, output: 'Focus session started: "Implement JWT refresh" (90 min, until 15:42)' },
  },
  {
    name: 'focus_status',
    category: 'focus',
    tier: 'hosted',
    tags: ['productivity', 'read-only'],
    description: 'Show the current focus session status: goal, time remaining, and parked distractions.',
    params: [],
    example: { input: {}, output: 'Focus session active: "Implement JWT refresh"\nTime remaining: 52 min\nParked items: 2 distractions' },
  },
  {
    name: 'focus_park',
    category: 'focus',
    tier: 'hosted',
    tags: ['productivity', 'write'],
    description: 'Park a distraction during a focus session without breaking concentration.',
    params: [
      { name: 'item', type: 'string', required: true, description: 'Distraction to park (e.g. "Review PR #123")' },
    ],
    example: { input: { item: 'Review PR #123' }, output: 'Parked: "Review PR #123". Focus maintained. (parked_items: 3)' },
  },
  {
    name: 'focus_end',
    category: 'focus',
    tier: 'hosted',
    tags: ['productivity', 'write'],
    description: 'End the current focus session and return a summary plus parked items.',
    params: [],
    example: { input: {}, output: 'Session ended.\nGoal: "Implement JWT refresh"\nDuration: 87 min (completed)\nParked items:\n• Review PR #123\n• Update docs' },
  },
  {
    name: 'focus_config',
    category: 'focus',
    tier: 'hosted',
    tags: ['productivity', 'config'],
    description: 'View or update focus session defaults (duration, prefix mode).',
    params: [
      { name: 'default_duration', type: 'number', required: false, description: 'Default session duration in minutes' },
    ],
    example: { input: { default_duration: 45 }, output: 'Focus config updated: default_duration=45 min' },
  },
  ...
  // ── Learning ───────────────────────────────────────────────────────────────
  {
    ...
    description: 'Start or resume a language learning session (programming or spoken language).',
    params: [
      { name: 'language', type: 'string', required: true, description: 'Language code (e.g. go, python, spanish, french)' },
      { name: 'level', type: 'string', required: false, description: 'Starting level (beginner, intermediate, advanced)' },
    ],
    example: { input: { language: 'go', level: 'intermediate' }, output: 'Learning session started: Go Intermediate\nNext exercise: "Type assertions"' },
  },
  {
    name: 'learn_status',
    category: 'learning',
    tier: 'hosted',
    tags: ['education', 'read-only'],
    description: 'Show learning progress across all languages with completion percentages and streaks.',
    params: [],
    example: { input: {}, output: 'Learning Progress:\n• Go (intermediate): 42% complete, 7-day streak\n• Python (advanced): 68% complete, 3-day streak' },
  },
  {
    name: 'learn_exercise',
    category: 'learning',
    tier: 'hosted',
    tags: ['education', 'read-only'],
    description: 'Get context and instructions for the next exercise in the current learning session.',
    params: [
      { name: 'language', type: 'string', required: true, description: 'Language code' },
    ],
    example: { input: { language: 'go' }, output: 'Exercise: Implement a custom type assertion\nInstructions: Create an interface and assert...\nHint: Use ok pattern' },
  },
  {
    name: 'learn_submit',
    category: 'learning',
    tier: 'hosted',
    tags: ['education', 'write'],
    description: 'Record an answer to a learning exercise and receive feedback.',
    params: [
      { name: 'language', type: 'string', required: true, description: 'Language code' },
      { name: 'answer', type: 'string', required: true, description: 'Your solution or answer' },
    ],
    example: { input: { language: 'go', answer: 'if v, ok := i.(MyType); ok { ... }' }, output: 'Correct! ✓ Move to next exercise.' },
  },
  {
    name: 'learn_schedule',
    category: 'learning',
    tier: 'hosted',
    tags: ['education', 'config'],
    description: 'View or update the daily learning schedule for a language.',
    params: [
      { name: 'language', type: 'string', required: true, description: 'Language code' },
      { name: 'daily_minutes', type: 'number', required: false, description: 'Target daily minutes' },
    ],
    example: { input: { language: 'go', daily_minutes: 30 }, output: 'Go schedule updated: 30 min/day target' },
  },
  ...
  // ── Sources ────────────────────────────────────────────────────────────────
  {
    ...
    tier: 'hosted',
    tags: ['monitoring', 'read-only'],
    description: 'List all watched sources (GitHub repos, RSS feeds, npm packages, etc.).',
    params: [],
    example: { input: {}, output: '5 sources:\n• repo: caboose-mcp/caboose-mcp (releases, last checked 1h ago)\n• npm: anthropic (latest version 1.2.0)' },
  },
  {
    name: 'source_add',
    category: 'sources',
    ...
    description: 'Add a source to watch: GitHub repo, RSS feed, URL, npm package, PyPI package, or GitHub user releases.',
    params: [
      { name: 'type', type: 'string', required: true, description: 'Source type: github_repo, rss, url, npm, pypi, github_user' },
      { name: 'path', type: 'string', required: true, description: 'Repository/feed path (e.g. owner/repo, https://..., package-name)' },
      { name: 'label', type: 'string', required: false, description: 'Optional display label' },
    ],
    example: { input: { type: 'github_repo', path: 'anthropics/anthropic-sdk-python' }, output: 'Added source: anthropics/anthropic-sdk-python (will check for releases)' },
  },
  {
    name: 'source_edit',
    category: 'sources',
    tier: 'hosted',
    tags: ['monitoring', 'write'],
    description: 'Edit an existing source configuration by ID.',
    params: [
      { name: 'id', type: 'string', required: true, description: 'Source ID from source_list' },
      { name: 'label', type: 'string', required: false, description: 'New display label' },
    ],
    example: { input: { id: 'src-001', label: 'Anthropic Python SDK' }, output: 'Source src-001 updated.' },
  },
  {
    name: 'source_remove',
    category: 'sources',
    tier: 'hosted',
    tags: ['monitoring', 'write'],
    description: 'Remove a source from the watch list.',
    params: [
      { name: 'id', type: 'string', required: true, description: 'Source ID from source_list' },
    ],
    example: { input: { id: 'src-001' }, output: 'Source src-001 removed from watch list.' },
  },
  {
    name: 'source_check',
    category: 'sources',
    tier: 'hosted',
    tags: ['monitoring', 'read-only'],
    description: 'Check one or all sources for new activity (releases, updates, posts).',
    params: [
      { name: 'id', type: 'string', required: false, description: 'Source ID to check (omit to check all)' },
    ],
    example: { input: {}, output: '🔔 caboose-mcp: v1.2.0 released (1h ago)\n🔔 anthropic-sdk-python: v0.28.0 available' },
  },
  {
    name: 'source_digest',
    category: 'sources',
    tier: 'hosted',
    tags: ['monitoring', 'read-only'],
    description: 'Check all sources and post a digest to Slack/Discord.',
    params: [
      { name: 'channel', type: 'string', required: false, description: 'Slack/Discord channel (default: configured channel)' },
    ],
    example: { input: {}, output: 'Digest posted to #notifications: 3 updates found' },
  },
]
